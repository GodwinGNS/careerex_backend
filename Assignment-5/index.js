...
const prescriptionDrugs = drugs.filter((eachDrug)=>{
    return eachDrug.isPrescriptionOnly === true; 
})
console.log(prescriptionDrugs);

// Return a new array, each item should follow the format: "Drug: [name] - [dosageMg]mg".

const returnDrug = drugs.map((newArr)=>{
    return `Drug: ${newArr.name} - ${newArr.dosageMg}mg `
})
console.log(returnDrug);

// Write a function that returns all drugs with a stock less than 50.

const stock = drugs.filter((stockArr)=>{
    return stockArr.stock < 50
})
console.log(stock);

// Return all drugs that are not prescription-only.

const notPrescription = drugs.filter((eachDrug)=>{
    return eachDrug.isPrescriptionOnly === false; 
})
...
const countDrugsByManufacturer = (manufacturerName) => 
    drugs.filter(drug => drug.manufacturer === manufacturerName).length;

const result = countDrugsByManufacturer("Pfizer");
console.log(result);

// Use forEach() to count how many drugs are Analgesics.
let count = 0

drugs.forEach((countDrug)=>{
    if (countDrug.category === "Analgesic" ){
        count++
    }     
})
console.log(count);